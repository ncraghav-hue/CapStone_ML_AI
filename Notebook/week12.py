...
def propose_next(X, y, func_id):
    ...
    pca.fit(X_scaled)
    principal_dirs = pca.components_

    # Generate candidate points along principal components
    n_candidates = 2000
    X_candidates = np.random.uniform(0,1,(n_candidates,dim))
    for i, dir_vec in enumerate(principal_dirs):

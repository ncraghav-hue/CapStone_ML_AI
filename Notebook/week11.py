...
def train_nn(X,y,hidden_layers,lr,epochs):
    model = ClusterNN(X.shape[1], hidden_layers)
...
# =============================
# PROPOSE NEXT QUERY USING CLUSTERS
# =============================
def propose_next(X, y, func_id, n_clusters=3):
    dim = X.shape[1]
    ...
    # Cluster analysis
    kmeans = KMeans(n_clusters=min(n_clusters,len(X)), random_state=42, n_init='auto').fit(X_scaled)
    cluster_centers = kmeans.cluster_centers_
    labels = kmeans.labels_

    # Generate candidates around cluster centers
    X_candidates = []
    for c in cluster_centers:
        X_local = c + scale*np.random.randn(local_n,dim)
        X_local = np.clip(X_local,0,1)
        X_candidates.append(X_local)
    ...
    cluster_target = kmeans.predict([X_candidates_scaled[best_idx]])[0]
    print(f"Function {func_id} targets cluster {cluster_target}")
